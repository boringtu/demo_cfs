            ...
            form(ref="fromline" class="form_wrap")
                .form_item
                    label
                        i *
                        span 客服ID
                    input(type="text" class="input_item" v-model="serverId" :disabled="IdIsDisabled" maxlength="20" @blur="checkIdIsRepeat")
                .form_item
                    label
                        i *
                        span 姓名
                    input(type="text" class="input_item" v-model="serverName" maxlength="20")
                .form_item
                    label
                        i *
                        span 昵称
                    input(type="text" class="input_item" v-model="serverNickName" maxlength="20")
                .form_item
                    label
                        i
                        span 角色
                    el-select(v-model="roleId" size="small" @change="roleChange")
                ...
                .form_item
                    label
                        i
                        span 所属分组
                    el-select(v-model="groupId" size="small")
                        el-option(v-for="item in allGroupList" :key="item.id" :value="item.id" :label="item.name")
                .form_item.pwd_item
                    label
                        i *
                        span 初始密码
                    input(type="password" class="input_item" v-model="password" minlength="8" maxlength="20")
                    .pwd_info
                        p 修改密码后需重新登录，请谨慎操作。
                ...
                .form_item
                    label
                        i *
                        span 确认密码
                    input(type="password" class="input_item" v-model="confirmPassword" minlength="8" maxlength="20")
                .submit_line
                    div(class="cancel_btn" @click="cancelSaveAccountInfo") 取消
                    div(class="save_btn" @click="saveAccountInfo") 保存
